...
class CashMoney {
  constructor() {
    ...
    this._HashMap = {};
    this._List = new libLinkedList(); // If the list gets over maxLength, we will automatically remove nodes on insertion.
  ...
  put(pData, pHash) {
    ...
    this._HashMap[tmpNode.Hash] = tmpNode; // Automatically prune if over length, but only prune this nodes worth.
  ...
  touch(pHash) {
    if (!this._HashMap.hasOwnProperty(pHash)) return false; // Get the old node out of the list
  ...
  expire(pHash) {
    ...
    let tmpNode = this._HashMap[pHash]; // Remove it from the list of cached records
  ...
  pruneBasedOnExpiration(fComplete, pRemovedRecords) {
    ...
    if (this.maxAge < 1) return fComplete(tmpRemovedRecords); // Now enumerate each record and remove any that are expired
  ...
  pruneBasedOnLength(fComplete, pRemovedRecords) {
    let tmpRemovedRecords = typeof pRemovedRecords === 'undefined' ? [] : pRemovedRecords; // Pop records off until we have reached maxLength unless it's 0
  ...
  pruneCustom(fComplete, fPruneFunction, pRemovedRecords) {
    ...
    for (let i = 0; i < tmpKeys.length; i++) {
      let tmpNode = this._HashMap[tmpKeys[i]]; // Expire the node if the passed in function returns true
  ...
  prune(fComplete) {
    let tmpRemovedRecords = []; // If there are no cached records, we are done.
...
class LinkedListNode {
  constructor() {
    this.Hash = false;
    this.Datum = false; // This is where expiration and other elements are stored;

    this.Metadata = {};
    this.LeftNode = false;
    this.RightNode = false; // To allow safe specialty operations on nodes
...
class LinkedList {
  ...
  initializeNode(pDatum, pHash) {
    ...
    this.totalNodes++; // Get (or create) a unique hash
  ...
  append(pDatum, pHash) {
    ...
    if (!tmpNode) return false; // The list just got longer!
  ...
  remove(pNode) {
    ...
    this.length--; // Last element in list.  Empty it out.
